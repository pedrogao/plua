...
impl Parser {
    ...
    fn local_declaration(&mut self) -> Result<Stmt, String> {
        let name = self.consume(TokenType::Identifier, "expect variable name")?.clone();
        let mut initializer = Expr::None;
        if self.match_token(TokenType::Equal) {
            initializer = self.expression()?;
        }
        let _ = self.consume(TokenType::Semicolon, "expect ';' after variable declaration")?;
    ...
    fn if_statement(&mut self) -> Result<Stmt, String> {
        ...
        Ok(Stmt::IfStmt(condition, Box::new(then_branch), Box::new(else_branch)))
    }
    ...
    fn assignment(&mut self) -> Result<Expr, String> {
        ...
        if self.match_token(TokenType::Equal) {
            ...
            return match expr {
                ...
                _ => {
                    Err(format!("{:?} invalid assignment target", equals))
                }
            };
        }
    ...
    fn comparison(&mut self) -> Result<Expr, String> {
        ...
        while self.match_tokens(vec![TokenType::Greater, TokenType::GreaterEqual,
    ...
    fn primary(&mut self) -> Result<Expr, String> {
        ...
        if self.match_token(TokenType::Number) {
            return Ok(Expr::Literal(self.previous().value));
        }
...
mod tests {
    ...
    fn test_parse_expr() {
        ...
        assert_eq!(tokens.as_ref().unwrap().len(), 12);
